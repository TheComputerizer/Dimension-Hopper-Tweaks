...
public abstract class MixinCommandTP extends CommandBase {
    ...
    private static void teleportEntityToCoordinates(
            Entity entity, CoordinateArg argX, CoordinateArg argY, CoordinateArg argZ, CoordinateArg argYaw,
            CoordinateArg argPitch) {}
        ...
        if(args.length<1) throw new WrongUsageException("commands.tp.usage");
            ...
            if(args.length!=1 && args.length!=2) {
                if(args.length<i+3) throw new WrongUsageException("commands.tp.usage");
                    ...
                    int k = i+1;
                    CoordinateArg arg = parseCoordinate(entity.posX,args[i],true);
                    CoordinateArg arg1 = parseCoordinate(entity.posY,args[k++],-4096,4096,false);
                    CoordinateArg arg2 = parseCoordinate(entity.posZ,args[k++],true);
                    CoordinateArg arg3 = parseCoordinate(entity.rotationYaw,args.length>k ? args[k++] : "~", false);
                    CoordinateArg arg4 = parseCoordinate(entity.rotationPitch,args.length>k ? args[k] : "~", false);
