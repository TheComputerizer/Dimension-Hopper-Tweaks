...
import static lumien.randomthings.config.Numbers.TIME_IN_A_BOTTLE_SECOND;

@Mixin(ItemTimeInABottle.class)
public abstract class MixinItemTimeInABottle implements IItemTimeInABottle {

    @Unique
    private ItemTimeInABottle dimhoppertweaks$cast() {
        return (ItemTimeInABottle)(Object)this;
    }

    @Unique
    private boolean dimhoppertweaks$checkReversal(ItemStack stack, World world, Entity entity) {
        int dimension = world.provider.getDimension();
        if(!world.isRemote && (dimension==44 || dimension==45) && entity instanceof EntityPlayer) {
    ...
    @Unique
    private void dimhoppertweaks$updateTime(ItemStack stack, int time) {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int slot, boolean isSelected) {
        if (!world.isRemote) {
            int secondWorth = TIME_IN_A_BOTTLE_SECOND;
            ...
            if(world.getTotalWorldTime()%60L==0L && entity instanceof EntityPlayer) {
                ...
                for(int i=0; i<player.inventory.getSizeInventory(); i++) {
                    ...
                    if(invStack.getItem()==dimhoppertweaks$cast() && invStack!=stack) {
                        ...
                        if(myTime<theirTime) myTimeData.setInteger("storedTime",0);
                    }
                }
            }
        }
