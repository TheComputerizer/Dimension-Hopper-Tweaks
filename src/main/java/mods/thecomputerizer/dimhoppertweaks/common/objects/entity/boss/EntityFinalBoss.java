...
import mods.thecomputerizer.dimhoppertweaks.common.objects.entity.boss.phase.*;
...
public class EntityFinalBoss extends EntityLiving implements IAnimatable {
    ...
    public void setDead() {
        super.setDead();
        ClientHandler.FOG_DENSITY_OVERRIDE = -1f;
        this.onAddedToWorld();
    }
    ...
    public void onAddedToWorld() {
        ...
        ClientHandler.FOG_DENSITY_OVERRIDE = 0f;
    }
        ...
        if(amount>0) {
            if (!damageCooldown.containsKey(player) || damageCooldown.get(player).getValue() <= 0) {
                ...
                if (newHealth>0) {
                    ...
                    playSound(player.getPositionVector(),DimensionHopperSounds.SHORT_STATIC);
                    return true;
                } else {
