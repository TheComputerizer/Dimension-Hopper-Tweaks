        ...
        if(!world.isRemote && entity instanceof EntityPlayer && !(entity instanceof FakePlayer)) {
            ...
            for(int i=0; i<9; i++) {
                if(i!=slot) {
                    ...
                    if(stack.getItem()==TFItems.steeleaf_ingot || (stack.getItem()==TFItems.block_storage &&
                            stack.getMetadata()==CompressedVariant.STEELLEAF.ordinal()))
                        healPower+=stack.getCount()/2;
                    else if(TFItemStackUtils.hasToolMaterial(stack,TFItems.TOOL_STEELEAF)) healPower++;
                }
            }
