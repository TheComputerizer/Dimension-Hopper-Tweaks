...
public class ClientEvents {
    ...
    public static List<Entry<Potion,Integer>> autoPotionItems = new ArrayList<>();
    ...
    @SubscribeEvent(priority=LOWEST)
    public static void onRenderBossBars(BossInfo event) {
        if(event.isCanceled()) return;
        if(event.getY()>12 && event.getY()>=event.getResolution().getScaledHeight()/5) event.setCanceled(true);
    }

    @SubscribeEvent(priority=LOWEST)
    public static void onPreTextRender(Pre event) {
        if(event.isCanceled()) return;
    ...
    public static void onGetTooltip(ItemTooltipEvent event) {
        ...
        Entry<Potion,Integer> validPotion = isValidPotion(event.getItemStack());
        if(Objects.nonNull(validPotion))
            event.getToolTip().add(TextUtil.getTranslated("trait.dimhoppertweaks.potion_master.auto_drink_enabled",
                    validPotion.getKey().getRegistryName(),validPotion.getValue()+1));
    }

    private static @Nullable Entry<Potion,Integer> isValidPotion(ItemStack stack) {
        for(PotionEffect effect : PotionUtils.getEffectsFromStack(stack))
            for(Entry<Potion,Integer> validPotion : autoPotionItems)
                if(effect.getPotion()==validPotion.getKey() && effect.getAmplifier()==validPotion.getValue())
                    return validPotion;
        return null;
    }
}